...
export class SheetsService {
      ...
      async () => {
        try {
          ...
          const guestStatuses = validatedResponse.guests
            .map((g) => (g.attending ? "attending" : "not_attending"))
            .join(", ");
          ...
          const derivedOverallStatus =
            attendingGuests.length === 0 ? "not_attending" : "attending";
  ...
  private parseRowToGuestEntry(row: any[]): GuestEntry {
    ...
    if (rsvpStatusValue.includes(",")) {
      ...
      rsvpStatus =
        (rsvpStatusValue as "pending" | "attending" | "not_attending") ||
        "pending";
