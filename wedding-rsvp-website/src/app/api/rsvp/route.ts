...
import type { APIResponse, RSVPFormData } from "@/types";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const existingEntry = await sheetsService.validateInvitationCode(
      ...
      return NextResponse.json(
        {
          ...
          message:
            "The provided invitation code was not found. Please check your code and try again.",
        } as APIResponse,
        { status: 404 },
      );
    }
    ...
    const isAlreadySubmitted = await sheetsService.isRSVPSubmitted(
      validatedData.invitationCode,
    );
    if (isAlreadySubmitted) {
      console.log(
        `RSVP update for invitation code: ${validatedData.invitationCode}`,
      );
    }

    // Validate that guest names match what's in the spreadsheet
    const expectedGuestNames = existingEntry.guestNames.map((name) =>
      name.trim().toLowerCase(),
    );
    const providedGuestNames = validatedData.guests.map((guest) =>
      guest.name.trim().toLowerCase(),
    );

    const namesMatch =
      expectedGuestNames.length === providedGuestNames.length &&
      ...
      return NextResponse.json(
        {
          ...
          message:
            "The guest names in your RSVP do not match our records. Please contact us if you need to make changes.",
        } as APIResponse,
        { status: 400 },
      );
    }
    ...
    const sanitizedData: RSVPFormData = {
      ...
      speechInMinutes: validatedData.speechInMinutes,
    };
    ...
    let errorMessage =
      "Something went wrong while processing your RSVP. Please try again or contact us directly.";
    let statusCode = 500;
    ...
    if (error instanceof Error) {
      ...
      if (
        ...
        errorMessage =
          "Our system is experiencing high traffic. Please try again in a few minutes.";
      ...
      // Authentication/permission errors
        ...
        errorMessage =
          "There was a problem with our system. Please contact us directly to submit your RSVP.";
      ...
      // Validation errors
        ...
        errorMessage =
          "There was an issue with the form data. Please check your entries and try again.";
      ...
      // Spreadsheet/data errors
        ...
        errorMessage =
          "Unable to process your RSVP. Please contact us directly.";
  ...
  return NextResponse.json(
    {
      ...
      message:
        "GET method is not supported for RSVP submission. Use POST instead.",
    } as APIResponse,
    { status: 405 },
  );
}
  ...
  return NextResponse.json(
    {
      ...
      error: "Method not allowed",
      message:
        "PUT method is not supported. Use POST to submit or update RSVPs.",
    } as APIResponse,
    { status: 405 },
  );
}
  ...
  return NextResponse.json(
    {
      ...
      error: "Method not allowed",
      message:
        "DELETE method is not supported. Contact us directly to cancel your RSVP.",
    } as APIResponse,
    { status: 405 },
  );
}
